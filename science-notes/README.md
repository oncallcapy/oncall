...
3. **What the evidence shows:** study design, sample and setting, endpoint definitions, effect estimates and precision, with claim-level citations.
...
Match each factual claim to a source that actually supports its scope, population, endpoint and time horizon. Read the methods and results, not only the abstract. Report effect size and uncertainty before headline statistical significance. Where data permit, give absolute alongside relative effects and identify the baseline risk, denominator and follow-up; never invent a conversion when these are unavailable.
