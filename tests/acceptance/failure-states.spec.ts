...
test('every unresolved launch fact and its evidence stay explicitly unverified', async ({ page }) => {
  ...
  for (const label of labels) {
    ...
    await expect(row.locator('.status')).toHaveText('NOT YET VERIFIED');
    await expect(row.locator('.evidence')).toHaveText('Launch evidence / NOT YET VERIFIED');
...
test('all five selected pairs separate ACTIVE API observations from unverified PAR and routes', async ({ page }) => {
  ...
  for (const [symbol, company] of identities) {
    ...
    await expect(card.locator('dl > div').filter({ has: page.locator('dt', { hasText: 'Technical launch check' }) }).locator('dd')).toHaveText('NOT YET VERIFIED');
    ...
    for (const label of ['PAR priceability', 'Route / in-range depth', 'ONCALL pool / launch receipt']) {
      await expect(card.locator('dl > div').filter({ has: page.locator('dt', { hasText: label }) }).locator('dd')).toHaveText('NOT YET VERIFIED');
    }
...
test('reduced motion hides opening controls and leaves case files accessible', async ({ page }) => {
