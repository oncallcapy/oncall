...
const excludedNames = [
  ...SELECTED_PAIR_IDENTITIES.flatMap(({ symbol, company }) => [symbol, company]),
  'Eli Lilly',
  'Lilly',
  'Hims & Hers',
  'UnitedHealth',
...
export const studyRecordSchema = z.strictObject({
  ...
  correction: nonBlank.min(12).optional(),
});

const selectedPairTickers = SELECTED_PAIR_IDENTITIES.map(({ symbol }) => symbol) as unknown as [
  typeof SELECTED_PAIR_IDENTITIES[number]['symbol'],
  ...typeof SELECTED_PAIR_IDENTITIES[number]['symbol'][],
];
const selectedPairSchema = z.strictObject({
  ticker: z.enum(selectedPairTickers),
  company: nonBlank.min(2),
}).refine(({ ticker, company }) => SELECTED_PAIR_IDENTITIES.some((pair) => pair.symbol === ticker && pair.company === company), {
  message: 'pair ticker and company must match one selected pair identity',
});

export const scienceDossierSchema = z.strictObject({
  slug: z.string().min(3),
  theme: scienceThemeIdSchema,
  pair: selectedPairSchema,
  title: evidenceFraming,
  dek: evidenceFraming,
  ...
  reviewer: nonBlank.min(3).optional(),
  reviewedAt: z.iso.date().optional(),
  correctionHistory: z.array(z.strictObject({ date: z.iso.date(), note: nonBlank.min(8) })).default([]),
...
export type ReviewedScienceDossier = ScienceDossier & { reviewer: string; reviewedAt: string };

// Content records may remain reviewable sources before human approval. Production
// deployment must check this predicate; a schema-valid dossier alone is not reviewed.
export const hasRecordedHumanReview = (dossier: ScienceDossier): dossier is ReviewedScienceDossier => Boolean(
  dossier.reviewer && dossier.reviewedAt,
);
